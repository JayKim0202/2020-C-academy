...
typedef struct _BITMAPINFOHEADER		// BMP 비트맵 정보 헤더 구조체(DIB 헤더)
{
	...
	int biYPersPerMeter;				// 그림의 세로 해상도(미터당 픽셀)
	unsigned int biClrUsed;				// 색상 테이블에서 실제로 사용되는 색상 수
	unsigned int biClrImportnat;		// 비트 맵을 표현하는 데 필요한 색상 인덱스 수
} BITMAPINFOHEADER;
...
#define PIXEL_SIZE 3		// 픽셀 한개의 크기 3바이트(24바이트)
#define PIXEL_ALIGN 4		// 필셀 데이터 가로 한 줄은 4의 배수 크기로 저장됨
...
int main()
{
	...
	unsigned char* image;	// 픽셀 테이터 포인터
	int size;				// 픽셀 데이터 크기
	...
	// 비트맵 파일 헤더 일기. 읽기에 실패하면 파일 포인터를 닫고 프로그램 종료
	...
	// 그리고 4에서 나머지를 뺴주면 남는 공간을 구할 수 있음
	// 만약 남는 공간이 10이라면 최종 결곽 4가 되믈 여기서 다시 4로 나머지를 구함
	padding = (PIXEL_ALIGN - ((width * PIXEL_SIZE) % PIXEL_ALIGN)) % PIXEL_ALIGN;

	if (size == 0)	// 픽셀 데이터 크기가 0이라면
	{
		// 이미지의 가로 크기 * 펙셀 크기에 남는 공간을 더해주면 완전한 가로 한 줄 크기가 나옴
		// 여기에 이미지의 세로 크기를 곱해주면 픽셀 데이더의 크기를 구할 수 있음
	...
	// 파일에서 픽셀 데이터 크기만큼 읽음. 일긱에 실패하면 파일 포인터를 닫고 프로그램 종료
	...
	// 픽셀 데이터는 아래위가 뒤집혀서 저장되므로 아래쪽부트 반복
	// 세로 크기만큼 반복
	for (int y = height - 1; y >= 0; y--)
	{
		...
		for (int x = 0; x < width; x++)
		{
			...
			// (x * 픽셀 크기)는 픽셀의 가로 위치
			// (y * (세로크기 * 픽셀 크기))는 픽셀이 몇 번째 줄인 지 계산
